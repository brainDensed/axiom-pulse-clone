...
import { PersonIcon, RocketIcon, LightningBoltIcon } from "@radix-ui/react-icons";
...
function Row({ token }: { token: PulseToken }) {
  ...
  const stats: TokenStat[] = useMemo(() => [
    ...
    },
    {
      ...
      highlightColor: 'var(--accent-9)' // Actually Row.tsx had default color for liquidity? No, it passed var(--accent-9) as defaultColor to useDirectFlash.
    },
    {
      label: "Vol",
      value: formatCurrency(token.volume),
      isCurrency: true,
      highlightColor: 'orange' // Row.tsx had Text color="orange"
    }
        ...
        <Theme appearance="dark">
          <Tooltip content="Holders" side="bottom">
            <Flex align="center" gap="1.5">
              ...
              <Text size="1" color="gray" style={{ fontSize: '11px' }}>{token.holders}</Text>
            </Flex>
          </Tooltip>
        </Theme>
        <Theme appearance="dark">
          <Tooltip content="Migrations" side="bottom">
            <Flex align="center" gap="1.5">
              ...
              <Text size="1" color="gray" style={{ fontSize: '11px' }}>{token.migrations}</Text>
            </Flex>
          </Tooltip>
        </Theme>
      </Flex>
